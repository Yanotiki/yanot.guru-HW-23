...
public class WikipediaAppAndroidTests extends TestBase {
    ...
    void checkTextOnSecondScreen() {
        ...
        step("Проверяю текст на экране", () -> {
            $(AppiumBy.id("org.wikipedia.alpha:id/primaryTextView")).shouldHave(attribute("text", "New ways to explore"));
            $(AppiumBy.id("org.wikipedia.alpha:id/secondaryTextView")).shouldHave(attribute("text", textOnScreenTwo));
        });
    }
    ...
    void checkTextOnThirdScreen() {
        ...
        step("Проверяю текст на экране", () -> {
            $(AppiumBy.id("org.wikipedia.alpha:id/primaryTextView")).shouldHave(attribute("text", "Reading lists with sync"));
            $(AppiumBy.id("org.wikipedia.alpha:id/secondaryTextView")).shouldHave(attribute("text", textOnScreenThree));
        });
    }
    ...
    void checkTextOnFourthScreen() {
        ...
        step("Проверяю текст на экране", () -> {
            $(AppiumBy.id("org.wikipedia.alpha:id/primaryTextView")).shouldHave(attribute("text", "Send anonymous data"));
            $(AppiumBy.id("org.wikipedia.alpha:id/secondaryTextView")).shouldHave(attribute("text", textOnScreenFour));
        });
        ...
        step("Проверяю, что свитч отправки данных выключен", () -> {
            $(AppiumBy.id("org.wikipedia.alpha:id/switchView")).shouldHave(attribute("checked", "false"));
        });
